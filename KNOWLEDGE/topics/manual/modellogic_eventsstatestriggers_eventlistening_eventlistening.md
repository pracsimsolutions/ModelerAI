---
id: modellogic_eventsstatestriggers_eventlistening_eventlistening
name: "Event Listening"
kind: manual
breadcrumb: ["ModelLogic","EventsStatesTriggers","EventListening","EventListening"]
tags: ["manual","modellogic","eventsstatestriggers","eventlistening"]
source: "manual/ModelLogic/EventsStatesTriggers/EventListening/EventListening.html"
---
...
1. **Adding Logic to an Event** - The Process Flow tool has event-listening activities that can listen for certain events in the 3D model and add custom logic that will be executed when this event occurs. You can also use FlexScript to add custom logic to 3D model events. Event listening is one of the most common ways to link a process flow to a 3D model.
1. **Collecting Statistics** - The Statistics Collector can listen to events in both the 3D model and the Process Flow tool and gather data from those events. This tool makes it possible to create custom statistics to meet your unique business system's analysis needs.
...
The Process Flow tool will be explained in the rest of this chapter. The Statistics Collector and other related concepts and tools are explained in the chapter about Getting Data from Your Model.
...
- **Dynamic Object** - You can set the object to be context-sensitive, meaning it will change based on certain conditions.
...
## Event Parameters and Label Assignment
...
### Statistics Collector Event Listening
...
You can set the object to be context-sensitive, meaning it will change based on certain conditions. One way to do this is to use the `current` keyword on any type of process flow except general process flows. The event-listening activity will listen to the object that is attached to each instance of that process flow. (See Process Flow Instances for more information.)
...
## Listening to Flow Items or Tokens
...
1. Run the simulation model so that at least one flow item and/or token is in the model.
1. With the process flow tab active, add a **Wait for Event** to the process flow.
1. Click the activity's icon to open its properties, as shown in the following image:
1. Next to the **Event** box, click the **Sampler** button to enter sampling mode. Click the flow item in the 3D model. This will open a menu that will list all of the possible events available for that object.
1. In the **Object** box, use a label that will reference the flow items or tokens dynamically.
...
## Listening to a Statistical Event or Change
...
- If you use the Arrive At Value, you'll need to specify which value needs to be arrived at in the *Value* box.
- You will need to check the *Fire If Initial Value Meets Rule* if you wan the first token to be able to meet the initial value.
