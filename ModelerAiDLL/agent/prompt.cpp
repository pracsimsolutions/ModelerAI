...
constexpr const char* kRules =
    ...
    "\n"
    "  PROCESSFLOW ACTIVITY VARIABLES (the valid `variable` names for "
    "modelerai_set_activity_variable / modelerai_get_activity_variable, "
    "filtered to SEMANTIC variables — visual/inherited noise omitted; "
    "modelerai_get_activity_info returns the full list per-instance):\n"
    "\n"
    "  Kind codes (these map to FlexSim's `DATATYPE_*` constants on the "
    "underlying node — read the per-variable doc for full semantics):\n"
    "    n   = NUMBER (1) — JSON number.\n"
    "    s   = STRING (2) — JSON string; either literal text OR a FlexScript "
    "body (per-variable doc says which; expression-mode bodies have a "
    "standard codeHeader auto-prepended).\n"
    "    c   = COUPLING (3) — pointer to another treenode. Pass "
    "{ref:\"SiblingActivity\"} (same PF) or {ref_pf:\"OtherFlow\"} "
    "(another PF).\n"
    "    x   = SIMPLE (7) — POLYMORPHIC: can hold a coupling pointer, a "
    "literal, OR a FlexScript expression. Common on `assignTo`, "
    "`objectRef`, `involved1`, `teRef`. Read the per-variable doc — input "
    "shape depends on what mode you want.\n"
    "    ct  = container (dataType=0) — a structured subtree, NOT a single "
    "value. Could be an SDT (FlexSim Structured Data Table — `events`, "
    "`eventBinding`), a Table (FlexScript Table API — ScheduleSource's "
    "`arrivals`), or a plain tree. `set_activity_variable` cannot set "
    "these; a dedicated tool is required (TODO — see modelerai-quickref "
    "gap list).\n"
    "\n"
    "  TokenCreation:\n"
    ...
    "  Basic:\n"
    "    AssignLabels           assignTo(x), includeDefaultLabels(n)\n"
    "    Breathe                delayTimeNode(s)\n"
    "    Delay                  delayTimeNode(s)\n"
    "    CustomCode             codeNode(s)\n"
    "    Decide                 decision(n)\n"
    "    Batch                  groupBy(c), batchQuantity(n), releaseQuantity(n), overflow(n), labelValues(ct)\n"
    "    WaitForEvent           events(ct), onListenerInitialized(s), assignObjectTo(c), decision(n)\n"
    "    CreateTokens           quantity(n), copy(n), parentLink(n), destination(c), holdToken(n), oneAtATime(n), assignTo(ct)\n"
    "    Milestone              involved(x), milestoneName(s), collector(c)\n"
    "    Sink                   deallocateAssets(n), preserveData(n)\n"
    "  Visual:\n"
    "    ChangeVisual           (no semantic vars; uses on-set FlexScript)\n"
    "    RunAnimation           objectRef(x), animation(n), waitUntilFinish(n), speedType(n), speedValue(n), repeatType(n), repeatValue(n)\n"
    "    CreateKinematics       dataNode(x), object(x), defineStart(n), xLoc(n), yLoc(n), zLoc(n), xRot(n), yRot(n), zRot(n)\n"
    "    ApplyKinematics        dataNode(x), object(x), type(n), xLoc(n), yLoc(n), zLoc(n), targetSpeed(n), acceleration(n), deceleration(n), startSpeed(n), endSpeed(n), startTime(n), assignType(n), waitForFinish(n), assignTo(ct)\n"
    "    FinishKinematics       dataNode(x), object(ct)\n"
    "  Objects:\n"
    "    CreateObject           objectRef(n), quantity(n), destination(s), assignType(n), assignTo(x)\n"
    "    MoveObject             objectRef(x), destination(x)\n"
    "    DestroyObject          objectRef(x), recycle(n), isAsynchronous(n)\n"
    "  Sub Flow:\n"
    "    RunSubFlow             labelAccess(n), holdToken(n), oneAtATime(n) — for `flowRef` use set_activity_variable with {ref_pf:\"OtherFlow\"} on the implicit flowRef variable\n"
    "    Start                  (no >variables — entry point)\n"
    "    Finish                 returnNode(ct)\n"
    "  TaskSequences:\n"
    "    CreateTS               teRef(x), assignType(n), assignTo(x), priority(n), preempt(n), waitForFinishState(n), dispatchAndWait(n), assignExecuterTo(x), callbackNode(s)\n"
    "    FinishTS               tsRef(x)\n"
    "    TETravel               taskType(n), involved1(x), var1(s)\n"
    "    TEAGVTravel            taskType(n), involved1(x)\n"
    "    TETimedTravel          taskType(n), involved1(x), var4(n)\n"
    "    TELoad                 taskType(n), involved1(x), involved2(ct)\n"
    "    TEUnload               taskType(n), involved1(x), involved2(x)\n"
    "    TEDelay                taskType(n), var1(n), var2(n)\n"
    "    TETravelToLoc          taskType(n), var4(s)\n"
    "    TETask                 callbackNode(s), involved1(ct), involved2(ct), resourceRef(x), var1..4(n), waitUntilFinish(n), taskType(n)\n"
    "    DispatchTS             teRef(x), tsRef(x), callbackNode(s), waitUntilFinish(n)\n"
    "  Shared Assets:\n"
    "    List                   listRef(c), pushCallback(s), pullCallback(s)\n"
    "    PushToList             listRef(c), push(x), partitionID(c), assignType(n), assignTo(x), keepOnList(n)\n"
    "    PullFromList           listRef(c), query(s), allOrNothing(n), doNotRemove(n), partitionID(c), puller(x), request(n), require(n), assignType(n), assignTo(x), keepBackorder(n), startIdle(s)\n"
    "    Resource               reference(c), refCount(n), defaultQuery(s), pullCallback(s)\n"
    "    Acquire                resourceRef(c), queryNode(s), quantity(n), priority(n), assignTo(x)\n"
    "    Release                resource(x), requirement(n), assignTo(c), assignType(n)\n"
    "    Variable               variableRef(n), variableData(x)\n"
    "    GetVariable            variableRef(c), assignTo(x), assignType(n)\n"
    "    SetVariable            variableRef(c), value(n), rate(ct)\n"
    "    Zone                   subsets(ct), constraints(ct), queueStrategy(s), enforceQueueOrder(n), maxContent(n), hasMaxContent(n), allowMultipleRequestsBeforeCheck(n), customConstraintCheck(ct), trackUnmatched(n), partitionNode(s), partitionCalculations(ct)\n"
    "    EnterZone              zoneRef(c), enterForRef(x), onFail(s)\n"
    "    ExitZone               zoneRef(c), exitForRef(x)\n"
    "  Warehousing:\n"
    "    FindItem               queries(ct), assignTo(x), outbound(n)\n"
    "    FindSlot               queries(ct), queryParams(ct), assignType(n), assignTo(x), assignToItem(x), fifo(n)\n"
    "  Coordination:\n"
    "    Split                  labelAccess(n), copy(n), parentLink(n), assignTo(x)\n"
    "    Join                   partitionID(s), partitions(ct)\n"
    "    Synchronize            partitionID(s), partitions(ct)\n"
    "  Preemption:\n"
    "    SaveContext            tokenRef(x), labelName(s)\n"
    "    ReleaseToken           tokenRef(x), releaseTo(c)\n"
    "    RestoreContext         (no >variables)\n"
    "\n"
    "  For the FULL per-class variable list (including the visual/inherited "
    "noise the prompt omits — fontSize, traceOption, next, prev, etc.) call "
    "read_topic(\"modelerai/processflow-activity-variables\") or "
    "modelerai_get_activity_info on a concrete instance.\n"
    "\n"
